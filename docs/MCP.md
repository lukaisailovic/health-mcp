...
| `get_goals` | `{}` |
| `set_goals` | any subset of `{ kcal, protein_g, carb_g, fat_g, fiber_g, sugar_g, sat_fat_g, sodium_mg, hydration_ml }` as `{ min?, max? }` bounds, plain number (uses per-macro default direction), or null to clear. Plus optional `weight_kg_target: number\|null` and `tracked_macros` (ordered array, ≤ 5, of the macros shown as Today rings; kcal is always shown). |
