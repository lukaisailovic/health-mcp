...
```http
GET /api/goals
PUT /api/goals      # any subset of: kcal, protein_g, carb_g, fat_g, fiber_g, sugar_g, sat_fat_g, sodium_mg, hydration_ml — pass {min?, max?} bounds, a plain number (interpreted via per-macro default direction), or null to clear. weight_kg_target stays a single number; tracked_macros is an ordered array (≤ 5) of the macros shown as Today rings (kcal is always shown).
```
